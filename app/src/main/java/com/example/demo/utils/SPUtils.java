...
public class SPUtils {
    ...
    //上一次登录状态文件
    public static final String LAST_LOGIN_STATUS_FILE_NAME = "sp_last_login_status";
    ...
    //电视
    private static final String SEARCH_HISTORY_ITEM_SEPARATOR = "_search_history_item_separator_";
    private static final String KEY_SEARCH_HISTORY = "key_search_history";

    public static void saveSearchHistory(String file, String value) {
        if (RegexUtils.isErrorValue(value)) { //去除特殊字符
            Log.e(TAG, "saveSearchHistory failed value = " + value);
            return;
        }
        ...
        ArrayList<String> list = getSearchHistory(file);
        list.remove(value);

        for (int i = 0, N = Math.min(9, list.size()); i < N; i++) {
            builder.append(SEARCH_HISTORY_ITEM_SEPARATOR).append(list.get(i));
        }
        editor.putString(KEY_SEARCH_HISTORY, builder.toString()).apply();
    }

    public static ArrayList<String> getSearchHistory(String file) {
        SharedPreferences sp = getSp(file);
        ArrayList<String> data = new ArrayList<>();
        if (sp == null) {
            return data;
        }
        String history = sp.getString(KEY_SEARCH_HISTORY, null);
        if (history != null) {
            String[] split = history.split(SEARCH_HISTORY_ITEM_SEPARATOR);
            data.addAll(Arrays.asList(split));
        }
        return data;
    }

    public static void clearSearchHistory(String file) {
        SharedPreferences sp = getSp(file);
        if (sp == null) {
            return;
        }
