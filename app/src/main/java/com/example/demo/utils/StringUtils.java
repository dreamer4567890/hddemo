...
public class StringUtils {
     ...
     * @return
     */
    public static String subStrLength(String content, int length) {
        String newContent = content.substring(0, getJavaLength(content, length));
        return newContent;
    }
     ...
     * @return
     */
    public static int getJavaLength(String content, int length) {
        int result = 0;
        int tmpLength = 0;
        if (content != null) {
            char[] tmp = content.toCharArray();
        ...
        result = result > content.length() ? content.length() : result;
        return result;
    }

    private static boolean isChineseCharactor(char charactor) {
        if (isChineseByBlock(charactor) || isChinesePuctuation(charactor)) {
            return true;
        }
        return false;
    }
