...
public abstract class DividerGridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerGridItemDecoration(Context context, int unit, float lineWidth, @ColorInt int colorRGB) {
        this.lineWidth = (int) TypedValue.applyDimension(unit, lineWidth, context.getResources().getDisplayMetrics());
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(colorRGB);
        mPaint.setStyle(Paint.Style.FILL);
    }
    ...
    /*protected List<? extends RecyclerView.ItemDecoration> buildItemDecorations() {
        int color = getResources().getColor(R.color.group_line_background);
        return Collections.singletonList(new DividerGridItemDecoration(this, TypedValue.COMPLEX_UNIT_PX, 1, color) {
            @Override
            public boolean[] getItemSidesIsHaveOffsets(int itemPosition) {
                //顺序:left, top, right, bottom
                boolean[] booleans = {false, false, false, false};
                switch ((itemPosition + 1) % 3) {
                    case 0:
                        //每一行第三个只显示左边距和下边距
                        booleans[0] = true;
                        booleans[3] = true;
                ...
                if (itemPosition == 6 || itemPosition == 7) {
                    booleans[3] = false;
                }
                return booleans;
            }
